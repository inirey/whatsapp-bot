...
sendButImage(from, `nihhh ngab🗿`, `lanjut?`, gim, [
      ...
      case "donasi":
         ...
         //RANDOM BY REY
            case 'caklontong':
				...
				Jawaban = '\n*'+randKey.result.desc +'*'
					...
					setTimeout( () => {
					rey.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					rey.sendMessage(from, '_20 Detik lagi_…', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					rey.sendMessage(from, '_30 Detik lagi_…', text) // ur cods
					}, 2500) // 1000 = 1s
				...
				case 'tebakgambar':
					...
					setTimeout( () => {
					sendButImage(from, `_jelaskan maksud dari gambar ini timer 30detik_`, `Made by Rey ❤️`, randSoal, [
            {
              buttonId: `${prefix}y`,
              buttonText: {
              ...
              type: 1,
				...
				Pertanyaan = randKey.result.soal
					...
					setTimeout( () => {
					sendButMessage(from, `${Pertanyaan}\n\n_waktu 30 detik dimulai dari sekarang_`, `Made by Rey ❤️ `, [
            {
             buttonId: `${prefix}y`,
               buttonText: {
                 displayText: `cancel game`,
          ...
          case "darkjokes":
                 ...
                 sendButImage(from, `darkjokers`, `Made by Rey ❤️`, darkjokes, [
            {
       ...
       // STAY HALAL BRADER
         ...
         teks = `• Doa: ${anu.result.name}\n• Arab: ${anu.result.arabic}\n• Latin: ${anu.result.latin}\n• Terjemahan: ${anu.result.terjemahan}`,
         sendButImage(from, `${teks}`, `Made by Rey ❤️`, buffer, [
            {
              buttonId: `${prefix}?`,
              buttonText: {
                displayText: `back menu`,
...
case 'quotemaker':
     ...
     case "aeshestic":
      ...
      if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓Sukses mengaktifkan mode anti jawa di group\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						if (!isJawa) return reply('Mode Anti jawa sudah Off Sebelumnya')
						_jawa.splice(from, 1)
						fs.writeFileSync('./lib/jawa.json', JSON.stringify(_jawa))
						reply(`\`\`\`✓Sukes menonaktifkan mode anti jawa di group\`\`\` *${groupMetadata.subject}*`)
					} else {
