        ...
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              let wel = await (await fetch(lw + `WELCOME`)).buffer()
              
              let lea = await (await fetch(lw + `LEAVE`)).buffer()

              conn.sendButtonLoc(jid, action === 'add' ? wel : lea, text, global.footer, 'okey', ',simi okey', false, {
                contextInfo: {
                  mentionedJid: [user]
                }
              })
